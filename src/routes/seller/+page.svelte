	...
	async function handleDelete(id: number) {
		...
		try {
			...
			if (res.ok && result.success) {
				...
				alert(result.error ?? `Failed to delete product (status ${res.status})`);
			}
		} catch (error) {
...
<div class="min-h-screen bg-gray-50">
	<div class="max-w-7xl mx-auto px-4 py-8">
		<div class="m-4">
			<Tabs items={['Products', 'Orders']} bind:value={activeTab} />
		</div>
...
{#if showEditModal && editingProduct}
	<div
		class="fixed inset-0 bg黑/50 flex items-center justify-center z-50"
